...
):BaseQuickAdapter<ProductReceivedItemModel, BaseViewHolder>(R.layout.item_assignment_report_items,data) {
    override fun convert(holder: BaseViewHolder, item: ProductReceivedItemModel) {
        with(holder){
            setText(R.id.item_assignment_report_product_name,item.productName)
            setText(R.id.item_assignment_report_product_quantity, "${item.quantity} ${item.units}")
        }
    }
}