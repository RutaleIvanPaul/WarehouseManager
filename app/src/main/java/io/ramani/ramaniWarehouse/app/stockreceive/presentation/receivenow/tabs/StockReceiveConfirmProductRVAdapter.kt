...
class StockReceiveConfirmProductRVAdapter(
    ...
    override fun convert(helper: BaseViewHolder, item: SelectedProductModel) {
        with(helper) {
            ...
            setText(R.id.item_product_confirm_row_agreed_amount, item.accepted.toString() + " " + item.units)
            setText(R.id.item_product_confirm_row_declined_amount, item.declined.toString() + " " + item.units)
