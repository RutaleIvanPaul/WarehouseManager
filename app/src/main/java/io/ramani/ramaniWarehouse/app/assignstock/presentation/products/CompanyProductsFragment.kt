...
class CompanyProductsFragment : BaseFragment() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        viewModel.start()
        companyProductsUIModelAdapter =
            CompanyProductsUIModelAdapter(companyProductsList, onItemClick = {
    ...
    private fun showDialog(item: ProductsUIModel) {
        ...
        val body =
            dialog.findViewById(R.id.layout_custom_rewards_dialogue_box_product_id) as TextView
        val primaryUnits =
            dialog.findViewById(R.id.dialogue_custom_forms_custom_price_button) as TextView
        val secondaryUnits = dialog.findViewById(R.id.dialogue_custom_forms_qty_button) as TextView
        val assignmentQuantity =
        ...
        viewModel.companyProductsListOriginal?.find { it._id == item._id }?.selectedUnits =
            item.units
...
//        }
        ...
        assignProductButton.setOnClickListener(View.OnClickListener {
            ...
            if (assignmentQuantity.text.trim().toString().isNullOrEmpty()) {
                ...
                Toast.makeText(context, R.string.record_assignment_valid_number, Toast.LENGTH_LONG)
                    .show()
            } else {

                viewModel.companyProductsListOriginal?.find { it._id == item._id }?.isAssigned =
                    true
                viewModel.companyProductsListOriginal?.find { it._id == item._id }?.assignedResource =
                    1
                viewModel.companyProductsListOriginal?.find { it._id == item._id }?.assignedResourceID =
                    R.drawable.assgn_button
                viewModel.companyProductsListOriginal?.find { it._id == item._id }?.displayText =
                    "Edit"
                ...
                total_assigned_products.setText(
                    "${
                        viewModel.companyProductsListOriginal?.filter { it.isAssigned == true }
                            .count()
                    } Assigned"
                )
